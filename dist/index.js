...
async function fetchNewsData(endpoint, params) {
    if (!API_KEY) {
        throw new Error("NEWSDATA_API_KEY environment variable is not set. Get your key at https://newsdata.io");
    }
    ...
    if (!response.ok) {
        ...
        if (response.status === 429) {
            throw new Error("Rate limit exceeded. Wait before making more requests or upgrade your plan.");
        }
...
const CommonNewsParams = {
    ...
    category: z
        .string()
        .optional()
        .describe("Comma-separated categories: business, crime, domestic, education, entertainment, environment, food, health, lifestyle, other, politics, science, sports, technology, top, tourism, world"),
    language: z
        .string()
        .optional()
...
function createServer() {
    ...
    // ── Tool: Latest News ──────────────────────────────────────────────────────
    server.registerTool("newsdata_latest", {
        title: "Latest News",
...
Examples:
  ...
  - Positive AI news: q="artificial intelligence", sentiment="positive"`,
        ...
        const data = await fetchNewsData("latest", params);
        const text = formatArticles(data);
        return { content: [{ type: "text", text }] };
    });
    // ── Tool: Crypto News ──────────────────────────────────────────────────────
    server.registerTool("newsdata_crypto", {
        title: "Crypto News",
...
Examples:
  ...
  - Negative crypto sentiment: sentiment="negative"`,
        ...
        const data = await fetchNewsData("crypto", params);
        const text = formatArticles(data);
        return { content: [{ type: "text", text }] };
    });
    // ── Tool: News Archive ─────────────────────────────────────────────────────
    server.registerTool("newsdata_archive", {
        title: "News Archive",
...
Examples:
  ...
  - Historical crypto coverage: q="bitcoin", from_date="2024-01-01", to_date="2024-12-31"`,
        ...
        const data = await fetchNewsData("archive", params);
        const text = formatArticles(data);
        return { content: [{ type: "text", text }] };
    });
    // ── Tool: Market News ──────────────────────────────────────────────────────
    server.registerTool("newsdata_market", {
        title: "Market News",
...
Examples:
  ...
  - Tech earnings: q="earnings report", category="business"`,
        ...
        const data = await fetchNewsData("market", params);
        const text = formatArticles(data);
        return { content: [{ type: "text", text }] };
    });
    // ── Tool: News Sources ─────────────────────────────────────────────────────
    server.registerTool("newsdata_sources", {
        title: "News Sources",
...
Examples:
  ...
  - Top-tier sources: prioritydomain="top"`,
        ...
        const data = await fetchNewsData("sources", params);
        const text = formatSources(data);
        return { content: [{ type: "text", text }] };
    });
    return server;
}
